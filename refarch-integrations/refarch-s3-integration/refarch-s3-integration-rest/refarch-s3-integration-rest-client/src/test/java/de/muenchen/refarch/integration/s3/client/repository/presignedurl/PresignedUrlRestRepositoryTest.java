...
class PresignedUrlRestRepositoryTest {
    ...
    void testGetPresignedUrlDeleteFile()
        ...
        Mockito.reset(this.fileApi);
        Mockito.when(this.fileApi.delete(PATH_TO_FILE, expireInMinutes)).thenReturn(Mono.just(expected));

        final String result = this.presignedUrlRestRepository.getPresignedUrlDeleteFile(PATH_TO_FILE, expireInMinutes);
        Mockito.verify(this.fileApi, Mockito.times(1)).delete(PATH_TO_FILE, expireInMinutes);
        assertThat(result, is(expected.getUrl()));
    }
    ...
    void testGetPresignedUrlDeleteFileException() {
        ...
        Mockito.reset(this.fileApi);
        Mockito.when(this.fileApi.delete(PATH_TO_FILE, expireInMinutes)).thenThrow(new HttpClientErrorException(HttpStatus.BAD_REQUEST));
        assertThrows(DocumentStorageClientErrorException.class,
                () -> this.presignedUrlRestRepository.getPresignedUrlDeleteFile(PATH_TO_FILE, expireInMinutes));
        Mockito.verify(this.fileApi, Mockito.times(1)).delete(PATH_TO_FILE, expireInMinutes);

        Mockito.reset(this.fileApi);
        Mockito.when(this.fileApi.delete(PATH_TO_FILE, expireInMinutes)).thenThrow(new HttpServerErrorException(HttpStatus.INTERNAL_SERVER_ERROR));
        assertThrows(DocumentStorageServerErrorException.class,
                () -> this.presignedUrlRestRepository.getPresignedUrlDeleteFile(PATH_TO_FILE, expireInMinutes));
        Mockito.verify(this.fileApi, Mockito.times(1)).delete(PATH_TO_FILE, expireInMinutes);

        Mockito.reset(this.fileApi);
        Mockito.when(this.fileApi.delete(PATH_TO_FILE, expireInMinutes)).thenThrow(new RestClientException(SOMETHING_HAPPENED));
        assertThrows(DocumentStorageException.class,
                () -> this.presignedUrlRestRepository.getPresignedUrlDeleteFile(PATH_TO_FILE, expireInMinutes));
        Mockito.verify(this.fileApi, Mockito.times(1)).delete(PATH_TO_FILE, expireInMinutes);
    }
