...
/**
 * Service, der einen OIDC /userinfo Endpoint aufruft (mit JWT Bearer Auth) und dort die enthaltenen "authorities" und "user_roles" extrahiert.
 */
@Slf4j
public class UserInfoAuthoritiesConverter implements Converter<Jwt, Collection<GrantedAuthority>> {
    ...
    /**
     * Ruft den /userinfo Endpoint und extrahiert {@link GrantedAuthority}s aus dem "authorities" Claim und roles aus dem "user_roles" claim.
     *
     * @param jwt der JWT
    ...
    public Collection<GrantedAuthority> loadAuthorities(final Jwt jwt) {
        ...
        try {
            @SuppressWarnings("unchecked") final Map<String, Object> map = this.restTemplate.exchange(this.userInfoUri, HttpMethod.GET, entity, Map.class)
                    .getBody();
