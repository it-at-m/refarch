...
import static de.muenchen.refarch.spring.security.SecurityConfiguration.SECURITY;
import static org.springframework.security.web.util.matcher.AntPathRequestMatcher.antMatcher;

import de.muenchen.refarch.spring.security.client.ClientParameters;
import de.muenchen.refarch.spring.security.userinfo.UserInfoAuthoritiesConverter;
...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain mainSecurityFilterChain(
        ...
        http
                .authorizeHttpRequests(authorize -> {
                    ...
                    Arrays.stream(springSecurityProperties.getPermittedUrls()).forEach(url -> authorize.requestMatchers(antMatcher(url)).permitAll());
     ...
     *                 It is required to map the information from the "authorities" field of the response from
     *                 UserInfo endpoint
     *                 to the Spring Granted Authorities. This implementation should remain <b>AS-IS</b>, until
     *                 mid-term refactoring of security.
     *                 Weak points:
     *                 - the converter should only convert instead of accessing the REST endpoints.
     *                 - the value of the endpoint URL is hard-coded based on config key-name and injected
     *                 via @Value instead of usage of OAuth2ClientProperties
     *                 - The Cache is configured internally in the service (check CacheConfiguration for other
     *                 caches)
     *                 - Consider this
     *                 https://docs.spring.io/spring-security/reference/servlet/oauth2/login/advanced.html which
     *                 states,
     ...
     *         - provide extension of the DEFAULT OAUth2 User Service see
